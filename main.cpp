// main.cpp
// Analog-horror-style console game (single-file)
// Compile: g++ main.cpp -std=c++17 -O2 -o analog_horror
// On Windows with MSVC: cl /EHsc main.cpp
...
void typeText(const string &s, int baseDelay = 25) {
        for (char c : s) {
                ...
                // occasional glitch characters inserted then removed
                if (randReal() < 0.02) {
...
// Simple UI: show status bar
void showStatus(int freq, int sanity) {
...
int main() {
        ...
        vector<string> intro = {
                ...
                "",
                "Press LEFT / RIGHT to tune",
                "Press SPACE to attempt lock",
                "Press Q to quit"
        };
        showBroadcastFrame(termW, termH, intro);
        cout << "\n";
        typeText("Establishing connection");
        for (int i=0;i<3;i++){
                cout << "." << flush; sleepMs(400);
        }
        sleepMs(600);
        ...
        while (running) {
                ticks++;
                // degrade sanity slowly
                if (!locked && ticks % 40 == 0) sanity = max(0, sanity - randInt(1,4));

                clearScreen();

                // display status (freq 0..999)
                showStatus(frequency, sanity);

                // At random, flicker a frame or broadcast text
                double mode = randReal();
                if (locked) {
                        // If locked, reveal narrative slowly
                        if (randReal() < 0.7) {
                                vector<string> story;
                        ...
                        if (mode < 0.35) {
                                ...
                                // partial fragment appears as glitched text
                ...
                // subtle beep (visual) border when near special freq ranges
                if (frequency % 37 == 0) {
                ...
                for (int dt = 0; dt < 40; dt += 10) {
                        if (_kbhit()) {
                                int c = _getch();
#if defined(_WIN32) || defined(_WIN64)
                                // arrow keys on Windows produce 0 or 224 then code
                                if (c == 0 || c == 224) {
                                        int k = _getch();
                                        if (k == 75) { frequency = max(0, frequency - randInt(1,8)); } // left
                                        else if (k == 77) { frequency = min(999, frequency + randInt(1,8)); } // right
                                } else
#endif
                                {
                                        if (c == 'q' || c == 'Q') { running = false; break; }
                                        if (c == ' ') {
                                                // Attempt to lock: success if frequency in "sweet spot"
                                                int secret = 173; // chosen seed frequency
                                                int dist = abs(frequency - secret);
                                                if (dist < 6 || (randReal() < max(0.0, 0.2 - dist*0.01))) {
                                                        locked = true;
                                                        currentFragment = fragments[randInt(0, (int)fragments.size()-1)];
                                                        // locking costs sanity
                                                        sanity = max(0, sanity - randInt(2,8));
                                                } else {
                                                        // failure: feedback and sanity drop
                                        ...
                                        } else if (c == 'a' || c == 'A') frequency = max(0, frequency - 1);
                                        else if (c == 'd' || c == 'D') frequency = min(999, frequency + 1);
                                }
                        }
                        sleepMs(10);
                }

                // If locked, sometimes reveal more and erode sanity
                if (locked && randReal() < 0.05) {
                        // reveal appended fragment bits
                ...
                // Win/escape condition: if locked and currentFragment long enough
